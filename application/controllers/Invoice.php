...
class Invoice extends CI_Controller
{
    ...
    public function form(){
        ...
        if(isset($_POST['preview'])){ // Jika user menekan tombol Preview pada form
          ...
          $upload = $this->Invoice_model->upload_file($this->filename);
    ...
    public function import(){
        ...
        foreach($sheet as $row){
          ...
          if($numrow > 1){
            ...
            array_push($data, array(
              'invoice_date'=>date("Y-m-d h:i:s", strtotime($row['A'])), // Insert data nis dari kolom A di excel
              'id_user'=>$row['B'], // Insert data nama dari kolom B di excel
              'pdam'=>$row['C'], // Insert data jenis kelamin dari kolom C di excel
              'listrik'=>$row['D'], // Insert data alamat dari kolom D di excel
            ));
          }
        ...
        $this->Invoice_model->insert_multiple($data);
        
        redirect("invoice"); // Redirect ke halaman awal (ke controller siswa fungsi index)
    }
}
