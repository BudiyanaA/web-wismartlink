...
class List_transaksi_toko extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('List_transaksi_toko_model');
    ...
    public function index()
    {
        ...
        $data['folder'] = 'list_transaksi_toko';
    ...
    {
        $row = $this->db->query("select e.id, us.nama, e.id_user, e.id_barang, r.nama_toko, u.nama_barang, u.harga, u.img, e.jumlah_barang, e.total_harga, e.status, e.created_date 
        from belanja_toko e 
        LEFT JOIN barang_toko u ON e.id_barang = u.id
        LEFT JOIN toko r ON u.id_toko = r.id
    ...
    function get_data()
    {
        $list = $this->List_transaksi_toko_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $field) {
            ...
            $row[] = '<td>
            <a href="' . base_url() . 'index.php/List_transaksi_toko/read/' . $field->id . '" class="btn btn-outline btn-circle btn-sm blue">
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->List_transaksi_toko_model->count_all(),
            "recordsFiltered" => $this->List_transaksi_toko_model->count_filtered(),
            "data" => $data,
        );
